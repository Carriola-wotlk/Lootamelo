LOOTAMELO_RESERVED_COLOR = "|cff00ccff";
LOOTAMELO_OFFLINE_COLOR = "|cffb0b0b0";
LOOTAMELO_WHITE_COLOR = "|cffffffff";

LOOTAMELO_PALADIN_COLOR = "|cfff58cba" -- paladin
LOOTAMELO_WARRIOR_COLOR = "|cffc79c6e" -- war
LOOTAMELO_MAGE_COLOR = "|cff40c7eb" -- mage
LOOTAMELO_HUNTER_COLOR = "|cffabd473" -- hunter
LOOTAMELO_DRUID_COLOR = "|cffff7d0a" -- druid
LOOTAMELO_ROGUE_COLOR = "|cfffff569" -- rogue
LOOTAMELO_PRIEST_COLOR = "|cffffffff" -- priest
LOOTAMELO_DEATHKNIGHT_COLOR = "|cffc41f3b" -- dk
LOOTAMELO_WARLOCK_COLOR = "|cff8787ed" -- warlock
LOOTAMELO_SHAMAN_COLOR = "|cff0070de" -- shaman

LOOTAMELO_RARE_ITEM = "|cffa335ee"

LOOTAMELO_WOW_ICONS_PATH = "Interface\\Icons\\";
LOOTAMELO_WOW_MAX_CHAT_CHAR = 255;
Lootamelo_main_frame = nil;
Lootamelo_PlayerLevel = 0;
Lootamelo_IsRaidOfficer = false;
Lootamelo_MasterLooterName = "";
Lootamelo_CurrentRaid = nil;
Lootamelo_Current_Page = nil;